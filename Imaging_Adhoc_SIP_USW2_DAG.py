...
FlywheelUpload = SSHOperator(
  ...
  ssh_conn_id='flywheel_usw2',
  command="fw ingest template s3://{{ dag_run.conf['precurated_bucket'] }}/{{ dag_run.conf['precurated_bucket_key'] }} --config-file {{ dag_run.conf['fw_template'] }} --group {{ dag_run.conf['fw_group'] }} --project {{ dag_run.conf['fw_project'] }} --cluster https://flywheel-us-sbx.science.roche.com/ingest -f",
  dag=dag,
)
